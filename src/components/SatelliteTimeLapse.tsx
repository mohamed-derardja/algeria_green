...
import { Sparkles, Calendar, Layers, ShieldCheck, ArrowLeftRight } from "lucide-react";
...
export default function SatelliteTimeLapse() {
  ...
  const locationData = {
    greendam: {
      ...
      subtitle: "Anti-Desertification Re-Afforestation Segment",
    ...
    aures: {
      ...
      subtitle: "Cedar of Lebanon Canopy Conservation",
      stats: "+88% Protected Cedar Density",
      desc: "Comparing Landsat-5 historical satellite bands with Sentinel-2 multispectral high-resolution imagery.",
    ...
    <section id="timelapse" className="py-20 px-container-padding bg-surface-container-low dark:bg-surface-container-lowest/70 border-b border-outline-variant/30">
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20 text-primary dark:text-primary-fixed text-xs font-semibold uppercase tracking-wider mb-3">
              ...
              16-Year Satellite Time-Lapse (2010 vs 2026)
            </div>
            <h2 className="font-headline-lg text-headline-lg text-on-background font-bold tracking-tight">
              Historical Vegetation Time-Lapse Comparison
            </h2>
            <p className="font-body-md text-on-surface-variant mt-2 max-w-2xl">
              Drag the interactive slider handle to compare historical satellite imagery against present-day Sentinel-2 multispectral canopy density.
            </p>
          </div>
        ...
        <div className="glass-card rounded-3xl p-6 border border-outline-variant/40 shadow-2xl relative overflow-hidden">
          ...
          <div className="relative w-full h-[420px] rounded-2xl overflow-hidden select-none border border-outline-variant/30">
            {/* 2026 Present Imagery (Base) */}
            <img
            ...
            {/* 2010 Past Imagery (Clipped Layer) */}
            <div
            ...
            >
              <div className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-9 h-9 rounded-full bg-primary text-on-primary shadow-2xl border-2 border-white flex items-center justify-center">
                <ArrowLeftRight className="w-4 h-4" />
              </div>
            </div>

            {/* Native HTML Range Input covering the canvas */}
            <input
              type="range"
              min="0"
              max="100"
